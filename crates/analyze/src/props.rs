...
fn collect_function_body_stmts<'a, 'b>(expr: &'a Expression<'b>, out: &mut Vec<&'a Statement<'b>>) {
    // Round-15 #2: walk every child expression, mirroring upstream's
    // `ts.forEachChild` descent. Pre-fix only Arrow / Function /
    // Parenthesised / CallExpression callee+args were pierced, so
    // common patterns like `const handlers = { save: () =>
    // dispatch('save') }` (object literal) or `const fns = [() =>
    // dispatch('save')]` (array) hid dispatcher activity from every
    // walker that consumes this helper.
    //
    // Stop at `ClassExpression` / `JSXElement` / `JSXFragment` —
    // they have their own scoping rules that the dispatcher
    // walkers don't currently model.
    match expr {
