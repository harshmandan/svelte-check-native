// === input.svelte ===
// generated by svelte-check-native; do not edit
async function $$render_0bc1cb85() {
    /** @type {any} */ const foo = /** @type {any} */ (undefined);
    void foo;
    /** @type {any} */ const bar = /** @type {any} */ (undefined);
    void bar;
    /** @type {any} */ const await_inside = /** @type {any} */ (undefined);
    void await_inside;
    /** @type {any} */ const defaultValue = /** @type {any} */ (undefined);
    void defaultValue;
    /** @type {any} */ const jsDoc = /** @type {any} */ (undefined);
    void jsDoc;
    ;(async () => {
        // template type-check body (incremental)
        {
            const foo = (x) => {
                { svelteHTML.createElement("div", {});                     (x);
                }
                void x;
                return null;
            };
            void foo;
            const bar = () => {
                { svelteHTML.createElement("div", {});                 }
                return null;
            };
            void bar;
            const await_inside = () => {
                { const $$_promise = (foo);
                ;(async () => {
                    const $$_await = await $$_promise; const bar = $$_await;
                    (bar);
                    void bar;
                }); }
                return null;
            };
            void await_inside;
            const defaultValue = (x = '') => {
                { svelteHTML.createElement("div", {});                     (x);
                }
                void x;
                return null;
            };
            void defaultValue;
            const jsDoc = (/**@type {number}*/a) => {
                (a);
                void a;
                return null;
            };
            void jsDoc;
            (foo(1));
            (bar());
            (await_inside());
            {
                const __svn_C_122 = __svn_ensure_component(Component);
                const __svn_inst_122 = new __svn_C_122({
                    target: __svn_any(),
                    props: {
                        bar: (x) => {
                            { svelteHTML.createElement("div", {});                                 (x);
                            }
                            void x;
                            return __svn_snippet_return();
                        },
                        children: () => __svn_snippet_return(),
                    },
                });
                /*svn:ignore_start*/const { bar } = __svn_inst_122.$$prop_def;/*svn:ignore_end*/
                /*svn:ignore_start*/void bar;/*svn:ignore_end*/
                { svelteHTML.createElement("div", {});                     (asd);
                }
            }
            {
                const __svn_C_181 = __svn_ensure_component(List);
                const __svn_inst_181 = new __svn_C_181({
                    target: __svn_any(),
                    props: {
                        "data": ([1, 2, 3]),
                        row: (item) => {
                            (item);
                            void item;
                            return __svn_snippet_return();
                        },
                        await_inside: () => {
                            { const $$_promise = (foo);
                            ;(async () => {
                                const $$_await = await $$_promise; const bar = $$_await;
                                (bar);
                                void bar;
                            }); }
                            return __svn_snippet_return();
                        },
                    },
                });
                /*svn:ignore_start*/const { row, await_inside } = __svn_inst_181.$$prop_def;/*svn:ignore_end*/
                /*svn:ignore_start*/void row;/*svn:ignore_end*/
                /*svn:ignore_start*/void await_inside;/*svn:ignore_end*/
            }
            {
                const __svn_C_219 = __svn_ensure_component(List);
                new __svn_C_219({ target: __svn_any(), props: {children: () => __svn_snippet_return()} });
            }
            {
                const __svn_C_23c = __svn_ensure_component(List);
                const __svn_inst_23c = new __svn_C_23c({
                    target: __svn_any(),
                    props: {
                        "data": ([1, 2, 3]),
                        row1: (item) => {
                            (item);
                            void item;
                            return __svn_snippet_return();
                        },
                        row2: (item) => {
                            (item);
                            void item;
                            return __svn_snippet_return();
                        },
                        children: () => __svn_snippet_return(),
                    },
                });
                /*svn:ignore_start*/const { row1, row2 } = __svn_inst_23c.$$prop_def;/*svn:ignore_end*/
                /*svn:ignore_start*/void row1;/*svn:ignore_end*/
                /*svn:ignore_start*/void row2;/*svn:ignore_end*/
                { svelteHTML.createElement("p", {});                 }
            }
            (children());
        }
    });
    return { props: /** @type {any} */({}), events: /** @type {{ [evt: string]: CustomEvent<any> }} */ ({}), slots: {}, bindings: /** @type {string} */ (''), exports: {} };
}
$$render_0bc1cb85;
/**
 * @typedef {Awaited<ReturnType<typeof $$render_0bc1cb85>>['props']} __SvnDefaultProps
 */
/**
 * @typedef {Awaited<ReturnType<typeof $$render_0bc1cb85>>['exports']} __SvnDefaultExports
 */
/** @type {import('svelte').Component<__SvnDefaultProps, __SvnDefaultExports>} */
export const __svn_component_default = /** @type {any} */ (null);
/** @typedef {ReturnType<typeof __svn_component_default>} __svn_component_default */
export default __svn_component_default;

