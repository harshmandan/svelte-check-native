...
/// Discover workspace member tsconfigs from a pnpm/npm/yarn workspace
...
///
/// When the user runs `svelte-check-native --workspace <monorepo-root>`,
/// our overlay-build chain anchored on the root tsconfig sees no
/// `paths` and no `references[]` to redirect via
/// [`flatten_references_from_chain`] — every `$lib/X` import then
/// fails with TS2307. This helper closes that gap by treating each
/// workspace member as an implicit reference; the overlay union of
/// member `paths` reaches each member's `$lib` mapping.
///
/// Limitation: when two members declare the SAME alias targeting
/// DIFFERENT directories (`$lib` in `apps/a` vs `apps/b`), the
/// overlay's combined value list lets tsgo try each in order. As long
/// as module names don't collide across sub-apps, both resolve
/// correctly. Cross-sub-app name collisions silently misresolve to
/// the first-listed sub-app — acceptable v0 tradeoff vs leaving every
/// `$lib` import unresolved.
///
/// pnpm-workspace.yaml parser is intentionally minimal — handles the
/// common `packages: [...]` shape with `- glob` items. Doesn't
/// understand quoted strings, anchors, or `!` exclusions; falls
/// through to empty when the file is absent or unparsable.
pub fn discover_workspace_member_refs(workspace_root: &Path) -> Vec<FlattenedReference> {
    let globs = read_workspace_member_globs(workspace_root);
    if globs.is_empty() {
        return Vec::new();
    }
    let mut out: Vec<FlattenedReference> = Vec::new();
    ...
    for glob in globs {
        for member_dir in expand_workspace_glob(workspace_root, &glob) {
            let cfg = member_dir.join("tsconfig.json");
            if !cfg.is_file() {
                continue;
            }
            if !seen_paths.insert(cfg.clone()) {
                continue;
            }
            // Skip the workspace root itself — it's already covered by
            // the entry chain.
            if member_dir == workspace_root {
                continue;
            }
            // resolve_reference reads the chain + paths/types/lib for
            // us. Pass the absolute member dir as the raw_path; the
            // function tolerates absolute paths.
            if let Some(r) =
                resolve_reference(member_dir.to_string_lossy().as_ref(), workspace_root)
            {
                out.push(r);
            }
        }
...
/// Read pnpm-workspace.yaml's `packages: [...]` list, falling back to
/// package.json's `workspaces` field (npm/yarn/bun convention).
/// Returns an empty Vec when neither exists or neither parseable.
fn read_workspace_member_globs(workspace_root: &Path) -> Vec<String> {
    if let Ok(content) = std::fs::read_to_string(workspace_root.join("pnpm-workspace.yaml")) {
        let parsed = parse_pnpm_workspace_yaml(&content);
        if !parsed.is_empty() {
            return parsed;
        }
    }
    if let Ok(content) = std::fs::read_to_string(workspace_root.join("package.json")) {
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
            // `workspaces` may be an array (npm/yarn classic) OR an
            // object with `packages` array (yarn workspaces v2 — same
            // shape as pnpm-workspace.yaml).
            let arr = json
                .get("workspaces")
...
/// Minimal pnpm-workspace.yaml parser. Reads:
///
/// ```yaml
/// packages:
///   - apps/*
...
/// ```
///
/// Strips surrounding `'` / `"`, ignores `!`-prefixed exclusion
/// patterns, ignores comment lines. Doesn't handle multi-line strings,
/// anchors, or other YAML features — they're rare in this file format.
fn parse_pnpm_workspace_yaml(content: &str) -> Vec<String> {
    ...
    for raw in content.lines() {
        ...
        if !line.starts_with(' ') && !line.starts_with('\t') {
            in_packages = trim.starts_with("packages:");
            continue;
        }
        if !in_packages {
            continue;
        }
        if let Some(rest) = trim.strip_prefix('-').map(str::trim) {
            let cleaned = rest
...
/// Expand a single workspace glob (`apps/*`, `packages/foo`,
/// `packages/course-app/src/*`) to absolute directory paths under
/// `root`. Only `*` at any segment is supported; `**` and brace
/// expansion are not handled (would require pulling in `globset`,
/// which we don't use elsewhere in core; keep this targeted at the
/// monorepo-glob convention).
fn expand_workspace_glob(root: &Path, glob: &str) -> Vec<PathBuf> {
    if glob.contains("**") {
        return Vec::new();
    }
    let parts: Vec<&str> = glob.split('/').filter(|s| !s.is_empty()).collect();
    let mut current_dirs: Vec<PathBuf> = vec![root.to_path_buf()];
    for part in parts {
