...
pub(crate) fn scan_jsdoc_typedef_name(script: &str) -> Option<String> {
...
///
/// `scan_jsdoc_props_typedef_keys` is kept for a future pass that
/// combines user-Props per-key types with destructure-derived shape.
pub(crate) fn should_synthesise_js_props(
