...
pub const NON_INTERACTIVE_ROLES: &[&str] = &[
    ...
    "tooltip",
    "progressbar",
    // Digital-publishing (`doc-*`) and graphics (`graphics-*`)
    // non-interactive roles. Derived from svelte's `non_interactive_roles`
    // formula (constants.js) against aria-query 5.3.2: non-abstract roles
    // whose superClass chain contains no `widget`/`window`. Omitting these
    // 40 made us diverge on `role="doc-*"` / `role="graphics-*"` elements.
    "doc-abstract",
    "doc-acknowledgments",
    "doc-afterword",
    "doc-appendix",
