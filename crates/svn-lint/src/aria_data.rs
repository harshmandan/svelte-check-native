...
use crate::a11y_constants::ARIA_PROPS;
...
pub fn aria_prop_bit(name: &str) -> Option<u32> {
    for (i, (k, _)) in ARIA_PROPS.iter().enumerate() {
        if *k == name {
...
pub fn role_props(role: &str) -> Option<(u64, &'static [&'static str])> {
    for (k, mask, req) in ROLE_PROPS {
        if *k == role {
            return Some((*mask, *req));
        }
    }
    None
}
...
/// Per-role (role_name, props_bitmask, required_props). Derived from
/// `aria-query@5.x`'s rolesMap — 127 non-abstract roles.
pub const ROLE_PROPS: &[(&str, u64, &[&str])] = &[
